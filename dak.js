...
function insertRowAfter(targetRow) {
    ...
    newRow.innerHTML = `
        <td class="row-number">${rowCount}</td>
        <td><input type="date" class="cell" required data-row="${targetIndex + 1}" data-field="date" placeholder="Enter date..."></td>
        <td>
    ...
    html2pdf()
        .from(element)
            .set({
                ...
                html2canvas: {
                    scale: 5, // Increase resolution (2x scaling for better quality)
                    useCORS: true, // Enable CORS for external resources
                    width: element.scrollWidth, // Use the full width of the element
                    height: element.scrollHeight, // Use the full height of the element
                },
                jsPDF: {
                    unit: 'mm',
                    format: 'a3', // Standard A4 size (210mm x 297mm)
                    orientation: 'portrait', // or 'landscape' for wider pages
